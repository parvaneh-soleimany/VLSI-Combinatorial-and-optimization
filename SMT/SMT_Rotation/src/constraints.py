...
def SMT(min_height, max_height, n, w, x, y):
    ...
    rot = [Int(f"rot{i}") for i in range(n)]
    
    #Define Solver
    Solver = Optimize()
    Solver.set(timeout=300000)
    
    Solver.add([And(rot[i] <= 1, rot[i] >= 0) for i in range(n)])
    
    #assign x and y to the arrays acceptable by the solver
    Solver.add([And(circuits_x[i] == (x[i] * (1 - rot[i]) + y[i] * rot[i]), circuits_y[i] == (y[i] * (1 - rot[i]) + x[i] * rot[i])) for i in range(n)])
    ...
    #....................................constraint for rotation........................................
    #Do not rotate squares and rectangles with height more that plate width
    Solver.add([If(Or(circuits_x[i] == circuits_y[i], circuits_y[i] > w),rot[i] == 0,True) for i in range(n)])
    ...
    if str(Solver.check()) == 'sat':
        ...
        sol_rot = [int(model.evaluate(rot[i]).as_string()) for i in range(n)]
        exec_time = time.time() - start
        sol = [sol_height, sol_pos_x, sol_pos_y, sol_rot, exec_time]
    else:
        sol = 'fail'
